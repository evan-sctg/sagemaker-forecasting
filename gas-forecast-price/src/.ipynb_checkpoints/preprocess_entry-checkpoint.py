...
def forecast(y_train: pd.Series, 
             y_test: pd.Series, 
             config: Dict[str, Any], 
    ...
    y_pred_covid = target_pipeline.predict(fh=ForecastingHorizon(y_test.index, is_relative=False))
...
def main():
    ...
    parser.add_argument('--test_mode', action='store_true', help='Reduce data size and do not save the data to disk for uploading if in test mode')
    ...
    data = pd.read_csv(os.path.join(config['preprocess_input'], 'gas_data.csv'), index_col=0)
    ...
    y_train, y_test = temporal_train_test_split(data, test_size=config['forecast_horizon'])
    logger.info(f'Train set period: {y_train.index.min().strftime("%Y-%m-%d")} to {y_train.index.max().strftime("%Y-%m-%d")}')
    logger.info(f'Test set period: {y_test.index.min().strftime("%Y-%m-%d")} to {y_test.index.max().strftime("%Y-%m-%d")}')

    logger.info('Forecasting Covid-19 data...')
    y_test_covid = y_train.loc[y_train['covid_forecast'], 'price_per_gallon']
    y_train_covid = y_train.loc[y_train.index < y_test_covid.index.min(), 'price_per_gallon']
    ...
    logger.info(f'Covid Train set period: {y_train_covid.index.min().strftime("%Y-%m-%d")} to {y_train_covid.index.max().strftime("%Y-%m-%d")}')
    logger.info(f'Covid Test set period: {y_test_covid.index.min().strftime("%Y-%m-%d")} to {y_test_covid.index.max().strftime("%Y-%m-%d")}')
    ...
    gas_price_forecast = pd.concat([
        # Values before the first Covid-19 data point
        y_train.loc[y_train.index < y_test_covid.index.min(), 'price_per_gallon'],
        # Forecast Covid-19 data
        y_pred_covid,
        # Values after the last Covid-19 data point
        y_train.loc[y_train.index > y_test_covid.index.max(), 'price_per_gallon']
    ], axis=0).astype(np.float64)

    gas_price_forecast.name = 'price_per_gallon_forecast'
    y_train = pd.concat([y_train, gas_price_forecast], axis=1)

    logger.info('Saving data...')
    if not args.test_mode:
        for data, name in zip([y_train, y_test], ['train', 'test']):
            data.drop(columns=['covid_forecast']).to_csv(os.path.join(config['preprocess_output'], f'{name}/{name}.csv'), index=True)
            logger.info(f'Saved {name} data to {os.path.join(config["preprocess_output"], f"{name}/{name}.csv")}')
